...
async function handler(event) {
  ...
  if (!code) {
    return { statusCode: 400, body: JSON.stringify({ error: "Missing ?code=<your Cfx.re join code>" }) };
  }
  ...
  try {
    ...
    return { statusCode: 502, body: JSON.stringify({ error: "Server unreachable", detail: String(err) }) };
  }
}
